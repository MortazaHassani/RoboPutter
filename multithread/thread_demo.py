...
from ArucoProc import ArucoProc
...
def threadBoth(source=0):
    ...
    video_getter = VideoGet(source).start()
    aruco_pro = ArucoProc(video_getter.frame).start()
    video_shower = VideoShow(aruco_pro.frame).start()
    cps = CountsPerSec().start()

    while True:
        if video_getter.stopped or video_shower.stopped or aruco_pro.stopped:
            video_shower.stop()
            aruco_pro.stop()
            video_getter.stop()
            break
        ...
        frame = video_getter.frame
        frame = putIterationsPerSec(frame, cps.countsPerSec())
        aruco_pro.frame = frame
        video_shower.frame = aruco_pro.frame
        cps.increment()
